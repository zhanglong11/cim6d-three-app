      ...
      <view class="tki-tree-view">
        <scroll-view class="tki-tree-view-sc" :scroll-y="true">
          <template v-for="(item, index) in treeList">
            <block v-if="!item.source.disabled" :key="index">
              <view
                class="tki-tree-item"
                :style="[
                  {
...
export default {
  ...
  methods: {
    ...
    _fixMultiple(index, item) {
      ...
      let selectLeavesNode = this.treeList.filter(item => item.checked)
      /*  console.log(selectLeavesNode)*/
      this.$emit('change', selectLeavesNode)
    },
    // 重置数据
    _reTreeList() {
      this.treeList.forEach((v, i) => {
    ...
    _initTree(range = this.range) {
      this.treeList = []
      if (this.isSimple) {
        let flatData = toTree(range, null, 'id', 'parentId')
        //this._renderTreeList(flatData)
        renderTreeList(this.treeList)(flatData, 'id', 'name')
      } else {
        //this._renderTreeList(range)
        renderTreeList(this.treeList)(range, 'id', 'name')
      }
      this.$nextTick(() => {
