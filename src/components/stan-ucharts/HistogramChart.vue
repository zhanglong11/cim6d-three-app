...
export default {
  ...
  props: {
    ...
    colors: {
      type: Array,
      default: () => {
        return ['#1890ff', '#2fc25b', '#facc14', '#f04864', '#8543e0', '#90ed7d']
      }
    }
  },
  data() {
    ...
    dataAs: {
      handler(val) {
        histogramuCharts[this.canvasId].updateData({
          ...val,
          categories: val[this.labelKey],
          series: val[this.valueKey]
        })
      }
    }
  },
  ...
  methods: {
    render() {
      let defaultOption = {
        ...
        padding: [0, 20, 5, 10], //画布填充边距，顺序为上右下左，同css，但必须4位
        ...
        yAxis: {
          ...
          format: val => {
            ...
            let { type, fixed, name, unit = '万' } =
              this.yAxisAs && this.yAxisAs.formatter
                ? Object.assign(defaultSetting, this.yAxisAs.formatter)
                : defaultSetting
            if (type == 'number') {
              return `${val.toFixed(fixed)}${name}${unit}`
            } else if (type == 'percent') {
              let newName = name || '%'
              return `${(val * 100).toFixed(fixed)}${name}${newName}${unit}`
            } else {
              return val.toFixed(0) + unit
            }
          },
          min: 0,
          title: (this.yAxisAs && this.yAxisAs.title) || '' //Y轴标题
        },
      ...
      optionAs[this.canvasId] = Object.assign(
        defaultOption,
        this.basicAs,
        this.xAxisAS,
        this.yAxisAS,
    ...
    touchstart(e) {
      ...
      histogramuCharts[this.canvasId].showToolTip(e, {
        ...
        format: function (item, category) {
          ...
          let newName
          let { type, fixed, name } =
            that.yAxisAs && that.yAxisAs.formatter
              ? Object.assign(defaultSetting, that.yAxisAs.formatter)
              : defaultSetting
