...
export default {
  ...
  methods: {
    //获得详情
    async getDetail() {
      let res = await api.getworkDetail(this.id)
      this.form = { ...res.data }
    },
    //草稿状态
    async saveCaoGao() {
      ...
      let num = this.id ? 2 : 1
      this.id && this.status !== '3' ? api.getworksUpdate(form) : await api.getworksSave(form)
    ...
    async goSubmit() {
      ...
      let num = this.id ? 2 : 1
      this.id && this.status !== '3' ? api.getworksUpdateAndSub(form) : await api.getworkAndSubmit(form)
      this.$utils.showToast('操作成功')
      this.isSave = true
      setTimeout(() => {
