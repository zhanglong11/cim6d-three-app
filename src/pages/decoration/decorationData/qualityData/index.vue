<template>
  <view class="main">
          ...
          <uni-card v-for="item in list" :key="item.id" @click.native="toDetail(item)">
            <view class="card-body">
              <view class="card-info">
                <view class="title">
                  <view class="ellipsis" style="max-width: 50%">{{ item.specificationName }}</view>
                </view>
    ...
    <uni-add v-if="$hasPower('AppQualityDataAdd')" @click.native="handleAdd" />
...
export default {
  ...
  methods: {
    ...
    toDetail(row) {
      this.$utils.toUrl('/pages/decoration/decorationData/qualityData/detail?id=' + row.id)
    },
    ...
    handleAdd() {
      this.$utils.toUrl('/pages/decoration/decorationData/qualityData/add')
    }
  }
}
</script>
