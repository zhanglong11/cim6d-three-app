  ...
  <view class="main">
    <view class="main-body">
      ...
      <uni-card>
        ...
        <uni-form-item label="月度：" :border="false" align="left">
          <view class="placeholder">{{ baseInfo.month }}</view>
        </uni-form-item>
        <uni-form-item label="编制人：" :border="false" align="left">
          <view class="placeholder">
            <span v-for="(item, index) in baseInfo.authorList" :key="index">{{ item.name + ',' }}</span>
          </view>
        </uni-form-item>
        <uni-form-item label="编制日期：" :border="false" align="left">
          <view class="placeholder">{{ baseInfo.reportDate | formatData }}</view>
        </uni-form-item>
      ...
      uni.showActionSheet({
        ...
        success: res => {
          ...
          if (index === 1) {
            ...
            this.$utils.toUrl(`/pages/decoration/decorationSupervisor/supervisorMonthly/add?id=${this.id}`)
          }
        }
      })
    }
