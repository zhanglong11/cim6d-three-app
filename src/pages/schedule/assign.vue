...
<script>
import { getScheduleTaskByTaskId, distributionScheduleTask } from './api'
export default {
  name: 'Assign',
  ...
  methods: {
    ...
    refresh() {
      getScheduleTaskByTaskId(this.form.taskId).then(res => {
        this.form = res.data
      })
    },
    // 提交
    async submit() {
      this.isLoading = true
      this.isLoading = true
      await distributionScheduleTask(this.form).finally(() => (this.isLoading = false))
