            ...
            <view class="card-body">
              <view class="card-info">
                ...
                <view class="flex-item">
                  <view class="flex-item-label">适用范围：</view>
                  <view class="flex-item-content">{{ item.applyRange }}</view>
                </view>
              </view>
              <!--<view>
                <p>
...
export default {
  ...
  methods: {
    refresh: _.debounce(async function () {
      ...
      let res = await Api.getImplementRulesList(data).finally(() => {
        this.loading = false
      })
    ...
    toDetail(row) {
      this.$utils.toUrl(`/pages/supervisorPlan/implementRules/detail?id=${row.id}&status=${row.status}`)
    },
    ...
    handleAdd() {
      this.$utils.toUrl('/pages/supervisorPlan/implementRules/add')
    }
  }
}
</script>
