<template>
  <view class="main">
    <uni-form-custom ref="form" class="main-body" :model="form" :rules="rules">
      <uni-card isForm>
        <uni-form-item label="监理规划名称" prop="name" isRequired>
          <textarea v-model.trim="form.name" auto-height placeholder="请输入监理规划名称" />
        </uni-form-item>
        ...
        <uni-form-item label="监理规划备注" prop="remark">
          <textarea v-model.trim="form.remark" auto-height placeholder="请输入监理规划备注" />
        </uni-form-item>
...
export default {
  ...
  methods: {
    ...
    onShow() {
      if (this.form.id) {
        this.getPlanRegistrationDetail()
        this.$utils.setTitle('编辑监理规划登记')
      }
    },
    getPlanRegistrationDetail() {
      Api.getPlanRegistrationDetail(this.form.id).then(res => {
        this.form = res.data
      })
    },
    // 保存
