...
export default {
  ...
  methods: {
    // 获取监理规划登记详情
    init() {
      Api.getPlanRegistrationDetail(this.id).then(res => {
      ...
      uni.showModal({
        ...
        success: res => {
          if (res.confirm) {
            ...
            Api.deletePlanRegistration(this.id).then(res => {
              uni.hideLoading()
              this.$utils.goBack()
            })
          } else if (res.cancel) {
      ...
      uni.showActionSheet({
        ...
        success: res => {
          ...
          if (index === 1) {
            ...
            this.$utils.toUrl(`/pages/supervisorPlan/planRegistration/add?id=${this.id}`)
          }
        }
      })
    },
