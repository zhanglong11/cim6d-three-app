<template>
  <div class="main is-white">
    ...
    <uni-add v-if="$hasPower('SafetyDangerAppAdd')" @click="handlerAdd"></uni-add>
    <!-- 筛选功能 -->
    <uni-drawer-custom ref="filterRef" @submit="handleDrawerFormChange" @reset="handleDrawerReset">
      <div>
        <h6 class="label">伤害类型</h6>
        <uni-checkbox-button-group v-model="drawerForm.harmCategory" hasAll :allValue="null" :options="harmTypeList" />
        <!--<p class="item-line" />
...
export default {
  ...
  methods: {
    refresh: _.debounce(async function () {
      ...
      let res = await Api.getDangerList(this.filterForm).finally(() => {
        this.loading = false
      })
    ...
    handlerAdd() {
      this.$utils.toUrl(`/pages/safety/danger/add`)
    },
    //searchBar的搜索事件
    ...
    toDetail(row) {
      this.$utils.toUrl(`/pages/safety/danger/detail?id=${row.id}`)
    }
  }
}
</script>
