...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        cycleEndDate: '',
        cycleStartDate: '',
        hiddenDangerCheckRecordDetailIds: ''
      },
      rules: {
      ...
      reportDate: [
        this.$utils.getDateWeek(moment(new Date()).format('YYYY-MM-DD')).first,
        this.$utils.getDateWeek(moment(new Date()).format('YYYY-MM-DD')).last
      ],
      status: '',
  ...
  methods: {
    ...
    //获取时间
    getReportDate(e) {
      this.form.cycleStartDate = moment(e[0]).format('YYYY-MM-DD')
      this.form.cycleEndDate = moment(e[1]).format('YYYY-MM-DD')
