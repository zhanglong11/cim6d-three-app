<template>
  <view class="main">
    <view class="main-body">
      <view>
        <view class="card">
          <uni-form-item label="检查日期" prop="checkDate">
            <view class="disabled">{{ form.checkDate | formatData }}</view>
          </uni-form-item>
          <uni-form-item label="检查组长" prop="checkGroupLeader">
            <view class="disabled">{{ form.checkGroupLeader }}</view>
          </uni-form-item>
          <uni-form-item label="检查组员" prop="checkGroupMembers">
            <view class="disabled">{{ form.checkGroupMembers }}</view>
          </uni-form-item>
          <uni-form-item label="建设单位" prop="buildCompany">
            <view class="disabled">{{ form.buildCompany }}</view>
          </uni-form-item>
          <uni-form-item label="施工单位" prop="constructionCompany">
            <view class="disabled">{{ form.constructionCompany }}</view>
          </uni-form-item>

          <uni-form-item label="监理单位" prop="supervisionCompany">
            <view class="disabled">{{ form.supervisionCompany }}</view>
          </uni-form-item>
        </view>
          ...
          <uni-form-custom :ref="index" :model="item" class="mt20" style="position: relative">
            <uni-card isForm>
              ...
              <uni-form-item class="file-item" label="是否合格" prop="qualifiedStatus">
                <view class="disabled">{{
                  item.qualifiedStatus ? (item.qualifiedStatus === 0 ? '不合格' : '合格') : ''
                }}</view>
              </uni-form-item>
...
export default {
  ...
  onLoad(options) {
    ...
    this.isDraft = Number(options.status) === 0 || Number(options.type === '1')
    this.init()
  },
  onReady() {
    // 非草稿状态隐藏顶部更多操作
