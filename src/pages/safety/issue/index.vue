<template>
  <div class="main is-white">
    ...
    <uni-add v-if="$hasPower('SafetyIssueAppAdd')" @click="handlerAdd"></uni-add>
    <!-- 筛选功能 -->
...
export default {
  ...
  methods: {
    ...
    handlerAdd() {
      this.$utils.toUrl(`/pages/safety/issue/add`)
    },
    //searchBar的搜索事件
    ...
    toDetail(row) {
      this.$utils.toUrl(`/pages/safety/issue/detail?id=${row.id}`)
    }
  }
}
</script>
