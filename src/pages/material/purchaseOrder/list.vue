  ...
  <view class="main">
          ...
          <uni-card v-for="item in list" :key="item.id" @click.native="handlePreviewItemInfo(item)">
            <view class="card-body">
              <view class="card-info">
                ...
                <view class="flex-item">
                  <view class="flex-item-label">申请到货时间：</view>
                  <view class="flex-item-content">{{ item.appyArrivalTime | formatData }}</view>
                </view>
              </view>
              <view>
                <p>
    ...
    <uni-drawer-custom ref="filterRef" @submit="handleDrawerFormChange" @reset="handleDrawerReset">
      <div>
        ...
        <uni-checkbox-button-group v-model="drawerForm.status" :options="purchaseStatus" />
        <!--        <h6 class="label">参与人员</h6>-->
        <!--        <uni-select-user v-model="drawerForm.participantId" :name.sync="drawerForm.keyword" :mode="2" multiple></uni-select-user>-->
      </div>
    </uni-drawer-custom>
  </view>
</template>
...
export default {
  ...
  methods: {
    refresh: _.debounce(async function () {
      ...
      Api.getPurchaseOrderList(data).then(res => {
    ...
    handleAddItemInfo() {
      this.$utils.toUrl('/pages/material/purchaseOrder/add')
    },
