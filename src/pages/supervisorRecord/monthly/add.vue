    ...
    <view class="main-body">
      <uni-form-custom ref="form" :model="form" :rules="rules">
        <uni-card isForm>
          ...
          <uni-form-item label="编制人" prop="writer" isLink>
            <uni-select-user v-model="form.writer" :name.sync="form.writerName" />
          </uni-form-item>
          <uni-form-item label="编制日期" prop="writeDate" isLink>
            <uni-date-picker v-model="form.writeDate" fields="day" mode="date"></uni-date-picker>
          </uni-form-item>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        writeDate: [{ required: true, message: '编制日期必选' }],
        month: [{ required: true, message: '月份必选' }],
        year: [{ required: true, message: '年度必选' }],
        writer: [{ required: true, message: '编制人必选' }]
      },
      isSave: false // 是否保存
    }
  },
